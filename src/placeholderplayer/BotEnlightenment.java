...
public class BotEnlightenment extends BotController {
    ...
    public static final RobotRole[] politicianRoleQ = {RobotRole.DEFENSE};
    public static int politicianRoleQIndex = 0;
        ...
        for (Direction dir : Utilities.directions) {
            if (rc.canBuildRobot(toBuild, dir, inf)) {
                ...
                if (toBuild == RobotType.POLITICIAN) {
                    RobotRole role = politicianRoleQ[politicianRoleQIndex];
                    politicianRoleQIndex = (politicianRoleQIndex + 1) % politicianRoleQ.length;
                    rc.setFlag(Flags.encodeRobotRole(rc.getRoundNum(), role));
                }
                break;
            }
        }
