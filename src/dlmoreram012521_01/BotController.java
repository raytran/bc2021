...
import dlmoreram012521_01.ds.CircularLinkedList;
import dlmoreram012521_01.flags.BoundaryType;
import dlmoreram012521_01.flags.EnemySpottedInfo;
import dlmoreram012521_01.flags.Flags;
import dlmoreram012521_01.nav.NavigationController;
...
public abstract class BotController {
    ...
    Optional<MapLocation> northEastCorner = Optional.empty();
    Optional<MapLocation> northWestCorner = Optional.empty();
    Optional<MapLocation> southEastCorner = Optional.empty();
    Optional<MapLocation> southWestCorner = Optional.empty();
    ...
    void searchForNearbyBoundaries() throws GameActionException {
        ...
        if (northBoundary.isPresent()) {
            if (eastBoundary.isPresent() && !northEastCorner.isPresent()) {
                northEastCorner = Optional.of(new MapLocation(eastBoundary.get(), northBoundary.get()));
            }
            if (westBoundary.isPresent() && !northWestCorner.isPresent()) {
                northWestCorner = Optional.of(new MapLocation(westBoundary.get(), northBoundary.get()));
            }
        }
        if (southBoundary.isPresent()) {
            if (eastBoundary.isPresent() && !southEastCorner.isPresent()) {
                southEastCorner = Optional.of(new MapLocation(eastBoundary.get(), southBoundary.get()));
            }
            if (westBoundary.isPresent() && !southWestCorner.isPresent()) {
                southWestCorner = Optional.of(new MapLocation(westBoundary.get(), southBoundary.get()));
            }
        }
    }
     ...
     */
    public Optional<MapLocation> getNorthEastCorner() {
     ...
     */
    public Optional<MapLocation> getNorthWestCorner() {
     ...
     */
    public Optional<MapLocation> getSouthEastCorner() {
     ...
     */
    public Optional<MapLocation> getSouthWestCorner() {
    ...
    /**
     * @return true if at least one corner is found
     */
    public boolean isCornerFound() {
        return     getNorthEastCorner().isPresent()
                || getNorthWestCorner().isPresent()
                || getSouthEastCorner().isPresent()
                || getSouthWestCorner().isPresent();
    }


    public MapLocation inCorner() {
        Optional<MapLocation> cornerLocation = Optional.empty();
        List<MapLocation> cornersToCheck = new LinkedList<>();
        if (northEastCorner.isPresent()) cornersToCheck.add(northEastCorner.get());
        if (northWestCorner.isPresent()) cornersToCheck.add(northWestCorner.get());
        if (southEastCorner.isPresent()) cornersToCheck.add(southEastCorner.get());
        if (southWestCorner.isPresent()) cornersToCheck.add(southWestCorner.get());

        for (MapLocation corner : cornersToCheck) {
            if (rc.canSenseLocation(corner)){
                cornerLocation = Optional.of(corner);
                break;
            }
        }
        return cornerLocation.orElse(null);
    }
