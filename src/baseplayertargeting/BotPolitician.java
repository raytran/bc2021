...
import baseplayertargeting.flags.*;
...
public class BotPolitician extends BotController {
    ...
    private void onEnemyNearby(RobotInfo robotInfo) throws GameActionException {
        ...
        if (!onlyTargetEC) {
            int bestTargetRadius = rc.getType().actionRadiusSquared;
            double sumOfEnemyInfluenceAtRadius = 0;
            int numEnemiesInRadius = 0;
            int numAlliesInRadius = 0;
            double bestAverage = 0;
            double currentAverage = 0;
            double ALLY_WEIGHT = 1;
            int numAlliesInBest = 0;
            int numEnemiesInBest = 0;
            RobotInfo[] robotsInAttackRange = rc.senseNearbyRobots();
            for(int i = 3; i <= rc.getType().actionRadiusSquared; i++){
                for(RobotInfo robot : robotsInAttackRange){
                    if(rc.getLocation().distanceSquaredTo(robot.getLocation()) == i){
                        if(robot.getTeam() != rc.getTeam()){
                            sumOfEnemyInfluenceAtRadius += robot.getInfluence();
                            numEnemiesInRadius++;
                        }
                        else{
                            sumOfEnemyInfluenceAtRadius -= robot.getInfluence() * ALLY_WEIGHT;
                            numAlliesInRadius++;
                        }
                    }
                }
                currentAverage = (double) sumOfEnemyInfluenceAtRadius / (numEnemiesInRadius + numAlliesInRadius);
                if(currentAverage >= bestAverage){
                    bestAverage = currentAverage;
                    bestTargetRadius = i;
                    numAlliesInBest = numAlliesInRadius;
                    numEnemiesInBest = numEnemiesInRadius;
                }
                sumOfEnemyInfluenceAtRadius = 0;
                numEnemiesInRadius = 0;
                numAlliesInRadius = 0;
            }
            System.out.println("Best Radius" + bestTargetRadius);
            int actionRadius = robotInfo.getType().equals(RobotType.ENLIGHTENMENT_CENTER) ? 2 : bestTargetRadius;
            if (robotInfo.location.distanceSquaredTo(rc.getLocation()) < actionRadius) {
                totalNearbyEnemyInfluence += robotInfo.influence;
                if (numEnemiesInBest >= numAlliesInBest && rc.canEmpower(actionRadius) && ((double) totalNearbyEnemyInfluence / rc.getInfluence() > .10 || (double) rc.getInfluence() / totalNearbyFriendlyInfluence < 0.25 || totalNearbyEnemyInfluence > 3)) {
