...
public class ECSpawnController implements ECController {
    ...
    public void run() throws GameActionException {
        ...
        if (toBuild.equals(RobotType.SLANDERER)){
            budget = (int) (rc.getInfluence() * 0.9);
            int i = 0;
        ...
        for (int i = 0; i < 8; i++) {
            if (rc.canBuildRobot(toBuild, nextSpawnDirection, buildAmount)) {
                ...
                if (toBuild.equals(RobotType.POLITICIAN) && buildAmount <= 21) {
                ...
                numSpawned++;
                break;
            } else if (rc.canBuildRobot(RobotType.MUCKRAKER, nextSpawnDirection, 1) && rc.getRoundNum() % 10 == 0) {
